define([
    ...
    let callbacks = [],
        ...
        action = function (cookieName) {
            ...
            let url = urlBuilder.build('visus_tfa/setup/recovery');

            return $.get(url).done(function (response) {
                if (!response.success) {
                    messageContainer.addErrorMessage({'message': $t('Recovery code retrieval has failed. Please refresh the page.')})
                }
